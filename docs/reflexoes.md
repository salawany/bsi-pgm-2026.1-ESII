...
Durante esta atividade, a suíte de testes foi a principal rede de segurança para confirmar que os refactorings não alteraram o comportamento do sistema. Após cada mudança, como a substituição do evento em formato de dicionário pela classe `Evento`, a renomeação da variável `atraso` para `dias_atraso` e a extração do método `_imprimir_atraso`, executei o `pytest` para verificar se tudo continuava funcionando. O fato de os testes permanecerem verdes indicou que as mudanças melhoraram a estrutura do código sem modificar o comportamento observado pelo usuário. Conforme discutido por Valente (Cap. 9), essa preservação do comportamento é o que caracteriza um refactoring. Sem testes, seria muito mais difícil ter confiança de que as alterações não introduziram erros.

Outro ponto importante foi o caso das subclasses `Notebook`, `Projetor` e `Cabo`. Após a aplicação do padrão Strategy na Aula 11, elas ficaram praticamente vazias, o que pode dar a impressão de um smell do tipo Data Class. Entretanto, nesse caso trata-se de um falso positivo. Essas classes continuam representando tipos diferentes de equipamentos e fazem parte da solução adotada para separar as regras de multa das entidades. Aplicar um refactoring como Inline Class removeria essa distinção e acabaria desfazendo parte da estrutura construída com o Strategy, reduzindo a flexibilidade obtida anteriormente.
